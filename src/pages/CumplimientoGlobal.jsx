// ============================================================
// Nivel de Cumplimiento — Tablero GLOBAL (solo superadmin)
// Matriz: sujetos obligados (filas) × 8 ítems + total (columnas).
// Usa la misma función de cálculo que el módulo por-sujeto
...
export default function CumplimientoGlobal() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const { data: tenants, error: eT } = await tenantsDeLaApp('*')
  ...
  function exportarExcel() {
    ...
    XLSX.writeFile(wb, `nivel-cumplimiento-global-${new Date().toISOString().slice(0, 10)}.xlsx`)
  }

  if (!isSuperAdmin) {
    return <div className="p-6 text-gray-500">Este tablero es exclusivo del superadministrador.</div>
  }
  if (loading) return <div className="p-6 text-gray-500">Calculando el nivel de cumplimiento de todos los sujetos obligados…</div>
  if (error) return <div className="p-6 text-red-600">Error al cargar: {error}</div>

  // Promedios por ítem (fila de totales)
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Nivel de Cumplimiento — Global</h1>
          <p className="text-sm text-gray-500">Estatus de todos los sujetos obligados, punto por punto. {filas.length} sujeto(s) obligado(s).</p>
        </div>
        <button onClick={exportarExcel} className="btn-secondary text-sm px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-50">
          ⬇ Exportar a Excel
        </button>
      </div>
